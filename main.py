...
def connect(point0, point1):
    ...
    if d0 > d1:
        return numpy.c_[
            ...
            numpy.round(numpy.linspace(ends[0, 1], ends[1, 1], d0 + 1)).astype(
                numpy.int32
            ),
        ]
    else:
        return numpy.c_[
            numpy.round(numpy.linspace(ends[0, 0], ends[1, 0], d1 + 1)).astype(
                numpy.int32
            ),
...
class Text:
    letters = {}
    font = pygame.image.load(resource_path("resources/font.png")).convert()
    for i in range(0, 64):
...
class ButtonSwitch:
    ...
    def __init__(self, row, heading, choices, default=0):
        ...
        self.choices_rects = [
            pygame.Rect(150, 30 * (i + first_row), 100, 30) for i in range(len(choices))
        ]
        self.index = default
        self.new = False
            ...
            if button.active:
                for i in range(len(button.choices_names)):
                    if ButtonSwitch.collide(button.choices_rects[i], mouse, True):
                        ...
                        mouse[0] = False
                    ButtonSwitch.draw(
                        button.choices_names[i],
                        button.choices_rects[i],
                        button.index == i,
                    )
    ...
    def draw(text, rect, active):
        ...
        Text.write(
            ...
            (fg, fg, fg),
            2,
            (rect.centerx - (len(text) * 8) // 2, rect.centery - 5),
        )
...
def draw_table(texts, coord):
    for line in range(len(texts)):
        ...
        pygame.draw.rect(
            Window.surface,
            (255, 255, 255),
            (coord[0], coord[1] + 30 * line, 100 * columns, 30),
        )
        pygame.draw.rect(
            Window.surface,
            (0, 0, 0),
        ...
        for column in range(columns):
            ...
            Text.write(
                ...
                (0, 0, 0),
                2,
                (center[0] - (len(text) * 8) // 2, center[1] - 5),
            )
...
class Weather:
    ...
    weather_name = ["Sun", "Clouds", "Rain", "Snow", "Storm", "Thunderstorm"]
    ...
    def update():
        if Weather.running:
            ...
            Weather.clouds = (
                noise(Weather.day, Weather.seeds[0], 0, 0.5)
                + abs(Weather.season - 2) / 4
            )
            ...
            Weather.temperature = (
                ...
                + Weather.clouds
                + abs(Weather.wind)
                + abs(abs(Weather.season - 2) - 2) * 3
                + abs(Weather.day % 1 - 0.5)
                + noise(Weather.day, Weather.seeds[2], -0.5, 0.5)
            ) / 10
            ...
            if Weather.fix is None:
                ...
                if (
                    abs(Weather.clouds - clouds)
                    + abs(Weather.temperature - temperature)
                    < 0.1
                ):
                    Weather.fix = None
            ...
            if Weather.weather != 0 and randbool(
                int(100 * (Weather.clouds / 2 + abs(Weather.wind) / 2))
            ):
...
class Sun:
    ...
    def update():
        Sun.coord[0] = (
            cos(Weather.day % 1 * 2 * pi - pi / 2) * Window.size[0]
            + Window.size[0] // 2
        )
        Sun.coord[1] = (
            sin(Weather.day % 1 * 2 * pi - pi / 2) * Window.size[0]
            + Window.size[1] * 1.4
        )
    ...
    def update():
        for cloud in Cloud.cloud:
            if Weather.running and Weather.weather_cycle:
                cloud.coord = (
                    cloud.coord[0] + Weather.wind * cloud.speed,
                    cloud.coord[1],
                )
            for i in range(cloud.size):
                x, y, r, c = cloud.points[i]
                d_point = abs(Sun.coord[0] - x - cloud.coord[0]) + abs(
                    Sun.coord[1] - y - cloud.coord[1]
                )
                d_cloud = abs(Sun.coord[0] - cloud.coord[0]) + abs(
                    Sun.coord[1] - (cloud.coord[1] - cloud.size // 2)
                )
                ...
                pygame.draw.circle(
                    Window.surface,
                    (color, color, color),
                    (x + cloud.coord[0], y + cloud.coord[1]),
                    r,
                )
            if Weather.running and Weather.weather_cycle:
                if Weather.weather == 2 and not randint(0, 20):
                    Material.create(
                        randcoord(
                            cloud.size // World.tile_size * 2,
                            cloud.size / 2 // World.tile_size,
                            (cloud.coord[0] - cloud.size) // World.tile_size,
                            (Window.size[1] - cloud.coord[1]) // World.tile_size,
                        ),
                        1,
                    )
                elif (
                    Weather.weather == 3
                    or Weather.weather > 1
                    and Weather.temperature < 0.2
                ) and not randint(0, 20):
                    Material.create(
                        randcoord(
                            cloud.size // World.tile_size * 2,
                            cloud.size / 2 // World.tile_size,
...
class Lightning:
    ...
    def update():
        for lightning in Lightning.lightning:
            strength = randfloat(
                lightning.points[-1][1] - Clock.dt, lightning.points[-1][1] + Clock.dt
            )
            ...
            lightning.angle = (
                (lightning.angle + lightning.origin_angle) % (2 * pi)
            ) / 2
            coord = (
                lightning.points[-1][0][0] + cos(lightning.angle) * strength * 3,
                lightning.points[-1][0][1] + sin(lightning.angle) * strength * 3,
            )
            if (
                strength < 0
                or lightning.time + lightning.points[-1][1] / 5 < Clock.time
            ):
                lightning.kill()
...
class Lightning:
    ...
    def update():
        for lightning in Lightning.lightning:
            ...
            coord = (
                lightning.lines[-1][-1][0][0] + cos(lightning.angle) * (strength + 3),
                lightning.lines[-1][-1][0][1] + sin(lightning.angle) * (strength + 3),
            )
            ...
            if randint(0, 5):
                lightning.lines.append(
                    [(lightning.strongest, 5), (lightning.strongest, 5)]
                )
            ...
            world_coord = (
                int(coord[0] / World.tile_size),
                World.world_size[1] - int(coord[1] / World.tile_size),
            )
    ...
    def explode(x, y, strength):
        ...
        for degree in range(0, 360, 10):
            ...
            for i in range(ceil(strength)):
                coord = (
                    floor(cos(degree) * (strength / 2) + x),
                    floor(sin(degree) * (strength / 2) + y),
                )
    ...
    def draw(self):
        color = max(
            150, min(255, 230 + randint(0, 10) - (Clock.time - self.time) * 100)
        )
        for points in self.lines:
            pygame.draw.lines(
                Window.surface,
                (color + 15, color + 15, color),
                False,
...
class World:
    ...
    def reset():
        World.world_size = (
            Window.size[0] // World.tile_size + 1,
            Window.size[1] // World.tile_size + 1,
        )
    ...
    def spawn_tree(x, y):
        ...
        for i in range(size + 1):
            for j in range(size + 1):
                if not (i, j) in ((0, 0), (0, size), (size, 0), (size, size)):
                    Material.create(
                        (int(i + x - size / 2), int(j + y + height - size / 2)), 6
                    )
        for i in range(height):
    ...
    def valid(coord):
        return (
            0 <= coord[0] < World.world_size[0] and 0 <= coord[1] < World.world_size[1]
        )
...
class Material:
        ...
        for x, y in numpy.argwhere(World.update_tiles):
            ...
            if World.world[x, y] == 1:  # Water
                ...
                if World.world[x, y] == 2:  # Snow
                    ...
                    if (
                        World.valid((x, y + 1))
                        and World.world[x, y + 1] == 1
                        and Weather.temperature > 0.4
                    ):
                        if not randint(0, 30):
                        ...
                        World.update_tiles[x, y], World.update_tiles[x, y + 1] = (
                            True,
                            True,
                        )
                    elif (
                        World.valid((x, y))
                        and World.valid((x, y + 2))
                        and World.world[x, y + 2] == 0
                        and World.valid((x - 4, y + 5))
                        and World.valid((x + 4, y + 5))
                        and World.world[x - 4, y + 5] == 0
                        and World.world[x + 4, y + 5] == 0
                        and Weather.temperature > 0.4
                        and not randint(0, 200)
                    ):
    ...
    def update_powder(x, y):
        ...
        destination = (x, y - 1)
        if (
            randbool(int(80 * abs(Weather.wind)))
            and World.valid((x + copysign(1, Weather.wind), y))
            and World.world[int(x + copysign(1, Weather.wind)), y] == 0
        ):
            found = True
            destination = (int(x + copysign(1, Weather.wind)), y)
        elif (
            World.valid(destination)
            and World.world[destination] <= 1
            and randint(0, 100)
        ):
            found = True
            World.velocity[x, y] = Material.density[World.world[x, y]]
        elif World.world[x, y] == World.world[x, y - 1] and randbool(
            100 - min(100, Material.density[World.world[x, y]])
        ):
            return
        else:
            if randint(0, 1):
            ...
            if (
                World.valid(destination)
                and World.world[destination] == 0
                and World.world[destination[0], destination[1] + 1] == 0
            ):
                found = True
                World.velocity[x, y] = Material.density[World.world[x, y]]
            else:
                ...
                if (
                    ...
                    if (
                        World.valid(destination)
                        and World.world[destination] < World.world[x, y]
                    ):
                        found = True
    ...
    def update_fluid(x, y):
        ...
        found = False
        if (
            randbool(int(90 * abs(Weather.wind)))
            and World.valid((x + copysign(1, Weather.wind), y))
            and World.world[int(x + copysign(1, Weather.wind)), y] == 0
        ):
            found = True
        ...
        if World.valid(destination) and World.world[destination] == 0:
            ...
            World.valid((x + 1, y - 1))
            and World.world[x + 1, y - 1] == 0
            and World.world[x + 1, y]
        ):
            found = True
            destination = (x + 1, y - 1)
        elif (
            ...
            if found:
                n = x - destination[0]
                if (
                    World.valid((destination[0], destination[1] - 1))
                    and 0 < World.world[destination[0], destination[1] - 1] < 8
                    and World.valid((destination[0] + n, destination[1] - 1))
                    and World.world[destination[0] + n, destination[1] - 1] == 0
                ):
                    (
                        World.world[destination[0] + n, destination[1] - 1],
                        World.world[destination[0], destination[1] - 1],
                    ) = (World.world[destination[0], destination[1] - 1], 0)
                    World.update_tiles[destination[0] + n, destination[1] - 1] = True
                    (
                        World.redraw[destination[0] + n, destination[1] - 1],
                        World.redraw[destination[0], destination[1] - 1],
                    ) = (True, True)
                if randint(0, 1):
                    for i in range(1, 3):
                        if (
                            World.valid((x + i * n, y))
                            and World.world[x + i * n, y] == 0
                        ):
                            destination = (x + i * n, y)
                        else:
                            break
        if found:
            World.world[destination], World.world[x, y] = World.world[x, y], 0
            World.velocity[destination], World.velocity[x, y] = (
                min(10, max(0, World.velocity[x, y] - 1)),
                0,
            )
            ...
            and not randint(0, 3)
            and World.valid((x, y + 1))
            and World.world[x, y + 1] == 0
            and World.world[x, y - 1] != 0
        ):
            World.world[x, y] = 2
            World.redraw[x, y] = True
            ...
            Weather.temperature > 0.4
            and not randint(0, 5)
            and not 0.2 < Weather.day % 1 < 0.8
        ):
            World.world[x, y] = 0
            World.redraw[x, y] = True
            for coord in ((x - 1, y + 1), (x, y + 1), (x + 1, y + 1)):
    ...
    def update_plant(x, y):
        ...
        if dead:
            ...
            World.valid((x, y + 1))
            and not World.world[x, y + 1] in (0, 1, 6, 7, 8)
            and World.world[x, y] == 7
        ):
            World.world[x, y] = 0
            World.redraw[x, y] = True
            World.update_tiles[x, y] = True
    ...
    def draw():
        for x, y in numpy.argwhere(World.redraw):
            ...
            pygame.draw.rect(
                ...
                (
                    World.tile_size * x,
                    Window.size[1] - World.tile_size * y,
                    World.tile_size,
                    World.tile_size,
                ),
            )
...
def main():
    ...
    b5 = ButtonSwitch(
        6, "Days per second", ("10", "5", "1", "0.5", "0.1", "0.05", "0.01", "Pause"), 5
    )
    ...
    while Window.running:
        ...
        if Weather.active:
            Window.surface.fill(
                (max(230 - 30 * abs(light - 0.5) * 2, 220), 220, 220),
                special_flags=BLEND_MIN,
            )
            light = min(0.9, max(0.1, light / 4 * 3 + 0.25))
            Window.surface.fill(
                (255 * light, 255 * light, 255 * light), special_flags=BLEND_MULT
            )
        ...
        if mouse[0]:
            Material.create(
                (
                    mouse[1][0] // World.tile_size,
                    World.world_size[1] - mouse[1][1] // World.tile_size - 1,
                ),
                selected,
            )

        if SHOW_FPS:
            info_table = [("FPS:", Clock.fps)]
        else:
            info_table = []
